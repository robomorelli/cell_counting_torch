...
class CellsLoader(Dataset):
    def __init__(self, images_path=None, masks_path=None, val_split=0.3, grayscale=False,
                 transform=None, ae=False, ae_bin=False, test=False,
                 priority_list=[], unlabelled=False, unweighted=False, limit=None):
        ...
        if self.unlabelled:
            ...
            self.imgs_list = priority_list
            self.mask_list = priority_list
        else:
            if limit != None:
                self.imgs_list = os.listdir(self.imgs_path)[:limit]
                self.mask_list = os.listdir(self.masks_path)[:limit]
            else:
                self.imgs_list = os.listdir(self.imgs_path)
    ...
    def __getitem__(self, idx):
        ...
        if self.unlabelled:
        ...
        elif self.ae == True or self.ae_bin == True and not self.test:
            ...
            if self.transform is not None:
                ...
                img = self.transform(img)
            if self.unweighted and not self.ae:
                mask = mask[0:1,:,:]
