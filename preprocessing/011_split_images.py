...
def main(args, color='y'):
    ...
    tot_num = len(os.listdir(args.image_path))
    AllImages = args.image_path
    AllMasks = (Path(args.image_path).parent.parent).as_posix() + '/all_masks/masks/'
    TrainValImages = (Path(args.image_path).parent.parent).as_posix() + '/train_val/full_size/all_images/images/'
    TrainValMasks = (Path(args.image_path).parent.parent).as_posix() + '/train_val/full_size/all_masks/masks/'
    TestImages = (Path(args.image_path).parent.parent).as_posix() + '/test/all_images/images/'
    TestMasks = (Path(args.image_path).parent.parent).as_posix() + '/test/all_masks/masks/'
    ...
    if color =='y':
        ...
        if args.random_test_set:
            ...
            test_names = [str(x) for x in test_names]

    else:
        NumberTest = int(tot_num*args.split_proportion[-1])
        UpperLimit = tot_num
        random.seed(123)  # FIX the SEED
        while len(test_names) < NumberTest:
            x = random.randint(0, UpperLimit)
            if images_name[x] not in test_names:
                test_names.append(images_name[x])
            else:
                continue
    ...
    # I failed to save in .TIF format when reading from original dataset_loader
    ...
    for ix, im_name in enumerate(images_name):
        ...
        print(im_name)
        if color=='red':
            mask_name = im_name.split('.')[0]+'_mask.tiff'
        else:
            mask_name=im_name
        ...
        if im_name in test_names:
            print('test')
            img_dir = TestImages + '{}'.format(im_name)
            mask_dir = TestMasks + '{}'.format(mask_name)
            ...
            mask_dir = TrainValMasks + '{}'.format(mask_name)
            plt.imsave(fname=img_dir, arr=np.squeeze(img_x))
            plt.imsave(fname=mask_dir, arr=np.squeeze(img_y), cmap='gray')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_path', nargs="?", default=AllImagesR, help='the folder including the masks to crop')
    ...
    parser.add_argument('--random_test_set', default=False,
                        help='use our test set instead of a random generated one')
    parser.add_argument('--split_proportion', default=[0.9, 0.1],
                        help='train_val proportion and test proportion as second element of the list')
